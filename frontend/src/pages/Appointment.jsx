    ...
    <div>
      <div className='flex flex-col sm:flex-row gap-4 '>
        <div className='w-full sm:w-56'>
          <img className='bg-[#189d01] w-full sm:max-w-72 rounded-lg ' src={docInfo.image} alt=""/>
        </div>
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center gap-3 w-full overflow-x-auto flex-nowrap mt-4'>
          {docSlots.length && docSlots[slotIndex].map((item,index)=>(
            <p onClick={()=>setSlotime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer overflow-hidden ${item.time === slotTime ? 'bg-[#189d01] text-white' :'text-gray-400 border border-gray-300' }`} key={index}>
              {item.time.toLowerCase()}
            </p>
          ))}
        </div>

        <button className='bg-[#189d01] text-white text-sm font-light px-14 py-3 rounded-full mt-4'>Book an appointment</button>
